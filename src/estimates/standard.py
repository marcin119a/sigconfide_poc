...
def findSigExposures(M, P, decomposition_method=decomposeQP):
    ...
    This function allows obtaining the optimal solution by specifying quadratic
    ...
    Returns:
        tuple: A tuple containing two numpy arrays.
            - exposures (numpy.ndarray): Matrix of signature exposures per
              sample/patient (column).
            - errors (numpy.ndarray): Estimation error for each sample/patient
              (Frobenius norm).

    Raises:
        ValueError: If 'M' and 'P' do not have the same number of rows
            (mutations types), or if 'P' has less than 2 columns, or if
            'decomposition_method' is not a function.
    ...
    Examples:
        ...
        E3 = findSigExposures(
            np.round(tumorBRCA * 10000), signaturesCOSMIC, decomposeQP
        )
    """
    ...
    if M.shape[0] != P.shape[0]:
        raise ValueError(
            "Matrices 'M' and 'P' must have the same number of rows (mutations types)."
        )
