...
from SigProfilerAssignment import Analyzer as SPA  # noqa: E402
...
def run_sigconfide(
    samples_df, gt_raw, cosmic_sub, sig_names, all_samples, mut_type, noise_level
):
    ...
    mandatory_idx = [
        sig_names_list.index(s) for s in mandatory_names if s in sig_names_list
    ]
    ...
    for sample in all_samples:
        ...
        tasks.append(
            (
                ...
                m,
                true_sigs,
                cosmic_sub.values.astype(float),
                sig_names,
                R,
                PRE_FILTER,
                mandatory_idx,
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as pool:
        ...
        for fut in as_completed(futs):
            ...
            if done % print_every == 0 or done == total:
                print(
                    f"    [sigconfide {mut_type}/{noise_level}] "
...
def run_spa(
    ...
    for sample in all_samples:
        ...
        pred = (
            ...
            or norm_map.get(_norm_spa(sample))
            and result[norm_map[_norm_spa(sample)]]
            or set()
        )
...
def run_round(mut_type, noise_level, cfg, max_samples, tmpdir):
    ...
    spa_agg = (
        _aggregate(combined.dropna(subset=["spa_f1"]), "spa")
        if spa_df is not None
        else {}
    )
...
def parse_args():
    ...
    p.add_argument(
        "--out-per-sample",
        default="compare_spa_results.csv",
        help="CSV file for per-sample results",
    )
    p.add_argument(
        "--out-summary",
        default="compare_spa_summary.csv",
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for mut_type in args.mut_types:
            ...
            for noise_level in args.noise_levels:
                ...
                print(
                    f"  sigconfide  mean: P={sc_mp:.3f}  R/sens={sc_mr:.3f}  "
                    f"F1={sc_mf1:.3f}  |  "
                ...
                if pd.notna(sp_mf1):
                    print(
                        f"  SPA         mean: P={sp_mp:.3f}  R/sens={sp_mr:.3f}  "
                        f"F1={sp_mf1:.3f}  |  "
