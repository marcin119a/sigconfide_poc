...
(sigconfide with --mandatory SBS1 SBS5), scm_od010 (the forced arm with the
over-dispersed bootstrap, --overdispersion 0.10) and spa.  Missing arms are
skipped.
...
ARMS = {
    ...
    "scm_od010": "sigconfide, forced + overdispersion 0.10",
    "spa": "SPA",
}
                ...
                rows.append(
                    {
                        ...
                        "mean_f1": float(s.mean_f1),
                        "mean_recall_weighted": float(s.mean_recall_weighted),
...
def md_prf(c: pd.DataFrame) -> str:
    ...
    out = [
        "| Ground truth | Level | " + " | ".join(ARMS[a] for a in arms) + " |",
        "| --- | --- | " + " | ".join("---" for _ in arms) + " |",
    ]
    for gt in GT_ORDER:
        for i, lv in enumerate(LEVELS):
            ...
            for a in arms:
                ...
                cells.append(
                    "-"
                    if r.empty
                    else f"{r.micro_precision.iloc[0]:.3f}"
                    f" / {r.micro_recall.iloc[0]:.3f}"
...
def md_fp(c: pd.DataFrame, gt: str) -> str:
    arms = [a for a in ARMS if a in set(c.arm)]
    out = [
        "| Level | " + " | ".join(ARMS[a] for a in arms) + " |",
        "| --- | " + " | ".join("---" for _ in arms) + " |",
    ]
    for lv in LEVELS:
        cells = []
        for a in arms:
            ...
            cells.append(
                "-"
                if r.empty
                else f"{r.fp_total.iloc[0]} ({r.fp_distractor.iloc[0]} distractors)"
            )
...
def md_burden(burden: pd.DataFrame, gt: str, lv: str) -> str:
    ...
    out = [
        "| Burden | n | " + " | ".join(ARMS[a] for a in arms) + " |",
        "| --- | --- | " + " | ".join("---" for _ in arms) + " |",
    ]
...
def md_per_signature(per_sig: pd.DataFrame, true: set[str], gt: str, lv: str) -> str:
    ...
    out = [
        "| Signature | tumours | "
        + " | ".join(f"{ARMS[a]} recall / FP" for a in arms)
        + " |",
        "| --- | --- | " + " | ".join("---" for _ in arms) + " |",
    ]
    for s in sigs:
...
def main() -> None:
    ...
    c.round(4).to_csv(args.set_dir / "comparison.csv", index=False)
    ...
    f1.to_csv(
        args.set_dir.parent / f"{args.set_dir.name.replace('-', '_')}_micro_f1.csv",
        index=False,
    )
    ...
    md = [
        ...
        "",
        f"{n} tumours, generated by make_diazgay_v36.py and scored by"
        " benchmark_isbs.py;"
