...
for sig, s in stats.items():
    sc_prec = (
        s["sc_tp"] / (s["sc_tp"] + s["sc_fp"])
        if (s["sc_tp"] + s["sc_fp"]) > 0
        else np.nan
    )
    sc_rec = (
        s["sc_tp"] / (s["sc_tp"] + s["sc_fn"])
        if (s["sc_tp"] + s["sc_fn"]) > 0
        else np.nan
    )
    spa_prec = (
        s["spa_tp"] / (s["spa_tp"] + s["spa_fp"])
        if (s["spa_tp"] + s["spa_fp"]) > 0
        else np.nan
    )
    spa_rec = (
        s["spa_tp"] / (s["spa_tp"] + s["spa_fn"])
        if (s["spa_tp"] + s["spa_fn"]) > 0
        else np.nan
    )
    ...
    rows.append(
        dict(
            ...
            recall_diff=(
                sc_rec - spa_rec
                if (not np.isnan(sc_rec) and not np.isnan(spa_rec))
                else np.nan
            ),
            prec_diff=(
                sc_prec - spa_prec
                if (not np.isnan(sc_prec) and not np.isnan(spa_prec))
                else np.nan
            ),
        )
    )
...
for mt in ["SBS", "DBS", "ID", "CN"]:
    ...
    for _, r in sub_sorted.iterrows():
        ...
        if not np.isnan(r.recall_diff):
            if r.recall_diff > 0.1:
                flag = "  ← SC better recall"
            if r.recall_diff < -0.1:
                flag = "  ← SPA better recall"
        print(
            FMT.format(
                r.signature,
                int(r.n_present),
